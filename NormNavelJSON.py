...
def add_role_to_category(role_name: str, category_name: str, optional_link:str = "") -> None:
    global role_table
    try:
        category = role_table[category_name]
        if category_name == "majors_and_degrees":
            category.append(role_name)
        else:
            category[role_name] = []
            print(category)
...
def add_link_to_role(link: str, role_name: str, category_name: str) -> None:
    global role_table
    try:
        server_list = role_table[category_name][role_name]
    ...
    for category_name in role_table:
        if category_name != "majors_and_degrees":
            for role_name in role_table[category_name]:
                role_table[category_name][role_name] = []
...
def get_servers_from_role(role_name: str, category_name: str) -> list:
    if category_name == "majors_and_degrees":
        ...
        return []
    try:
        return role_table[category_name][role_name]
    except KeyError:
...
def run_tests():
    ...
    add_role_to_category("CS141", "upper_div")
    print("adding role CS010A to role list under lower_div")
    add_role_to_category("CS010A", "lower_div")
    ...
    print("editing role name from CS141 to CS100")
    edit_role_name("CS141", "CS100", "upper_div")
    print(role_table)
    print("Removing https://discord.gg/lmao from CS100")
    remove_link_from_role("https://discord.gg/lmao", "CS100", "upper_div")
    print(role_table)
    ...
    add_link_to_role("https://discord.gg/lmao", "CS100", "upper_div")
    print(role_table)
